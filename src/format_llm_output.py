...
def main(request):
    ...
    extract_responses = lambda x: pd.Series(
        {
            "Question 1": x.split("Question 1:")[1].split("\n")[0].strip().replace("[", "").replace("]", "") if "Question 1:" in x else "", 
            "Question 2": x.split("Question 2:")[1].strip().replace("[", "").replace("]", "") if "Question 2:" in x else "",
        }
    )
    ...
    prompts_with_responses = pd.merge(
        left=prompts, right=llm_responses[["custom_id","llm_response_question1","llm_response_question2"]], on="custom_id"
    )
    data_with_responses = pd.merge(
        left=data, right=prompts_with_responses[["ID", "llm_response_question1", "llm_response_question2"]], on="ID", suffixes=("", "_y")
    )
...
if __name__ == "__main__":
    ...
    input_data = {
        ...
        "filename":"vaccine_financial_incentive_vaccinationstatus_llama3.1_8b_s2_updated.xlsx",
